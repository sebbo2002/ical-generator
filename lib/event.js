...
var ICalEvent = function(_data, calendar) {
    var attributes = ['id', 'uid', 'sequence', 'start', 'end', 'timezone', 'stamp', 'timestamp', 'allDay', 'floating', 'repeating', 'summary', 'location', 'description', 'organizer', 'attendees', 'alarms', 'method', 'status', 'url', 'transp'],
        vars,
        i,
        data;
    ...
    this.transp = function(transp) {
        ...
        if(vars.allowedTranspValues.indexOf(transp.toUpperCase()) === -1) {
            throw '`transp` must be one of the following: ' + vars.allowedTranspValues.join(', ') + '!';
        }

        data.transp = transp.toUpperCase();
        return this;
    };
    ...
    this.generate = function(calendar) {
        ...
        // TRANSPARENCY
        if(data.transp) {
            g += 'TRANSP:' + data.transp.toUpperCase() + '\r\n';
        }
