...
var ICalEvent = function(_data, calendar) {
    var attributes = ['id', 'uid', 'sequence', 'start', 'end', 'timezone', 'stamp', 'timestamp', 'allDay', 'floating', 'repeating', 'summary', 'location', 'description', 'htmlDescription', 'organizer', 'attendees', 'alarms', 'method', 'status', 'url', 'transparency'],
        vars,
        i,
        data;
    ...
    this.transparency = function(transparency) {
        ...
        if(vars.allowedTranspValues.indexOf(transparency.toUpperCase()) === -1) {
            throw '`transparency` must be one of the following: ' + vars.allowedTranspValues.join(', ') + '!';
        }

        data.transparency = transparency.toUpperCase();
        return this;
    };
    ...
    this.generate = function(calendar) {
        ...
        if(data.transparency) {
            g += 'TRANSP:' + data.transparency.toUpperCase() + '\r\n';
        }
