		...
		var cal = {
			...
			methods: {
				...
				setTTL: function(duration, unit) {
					...
					if (isNaN(duration)) {
						throw 'duration has to be a number';
					}

					if (!unitList[unit]) {
						throw 'unit "'+ unit+ '" is invalid or not supported';
					}

					cal.publishTTL = "PT" + duration + unitList[unit];
					return cal.methods;
				},
				addEvent: function(e) {
					var _event = {};
				...
				generate: function() {
					...
					if (cal.publishTTL)
						g += 'X-PUBLISHED-TTL:' + cal.publishTTL + '\n';
